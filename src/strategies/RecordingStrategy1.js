// src/strategies/RecordingStrategy.js (Final Version with De-duplication)
...
class RecordingStrategy {
  constructor() {
    ...
    // This Set is the key to de-duplicating steps
  ...
  async record(startUrl) {
    ...
    await page.addInitScript(() => {
      // The injected browser script with blur/click listeners
      ...
      const getElementText = (el) => {
        const text = el.innerText?.trim() || el.value?.trim() || el.name || el.getAttribute('aria-label') || '';
        return text.split('\n')[0].trim().slice(0, 50);
      };
    ...
    console.log("  -> Recording has started. Interact with the browser now.");
  ...
  _processAction(action) {
    ...
    if (!this.recordedLocators.has(action.selector)) {
        ...
        this.plan.locators.push({ name: locatorName, selector: action.selector, description: `Element for: '${action.text}'` });
    } else {
    ...
    // THIS IS THE CRITICAL FIX: Only add the step if its text is unique.
    if (step.text && !this.generatedStepTexts.has(step.text)) {
        this.generatedStepTexts.add(step.text);
        this.plan.steps.push(step);
    }
    // Always log the action so the user sees feedback.
    console.log(`  -> Recording Step: ${step.keyword || 'Action'} ${step.text || action.type}`);
  }
